...
export default function SchedulePitchModal({ isOpen, onClose }) {
  ...
  const [formData, setFormData] = useState({
    ...
    firm: '',
    email: '',
    meetingType: '30-Min Seed Pitch (Deck & Demo)',
    date: '',
    notes: ''
  });
      ...
      <div className="relative w-full max-w-lg bg-slate-950 border border-slate-800 rounded-3xl shadow-[0_0_80px_rgba(16,185,129,0.15)] overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-800/80 bg-slate-900/60">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-white">Schedule Investor Pitch</h3>
              <p className="text-xs text-slate-400">GridSense Seed Round • 1-on-1 Founder Briefing</p>
            </div>
          </div>
          <button
            onClick={resetForm}
        ...
        <div className="p-6 sm:p-8">
          {submitted ? (
            <div className="text-center py-6 space-y-4 animate-in zoom-in-95">
              ...
              <h4 className="text-2xl font-bold text-white">Pitch Slot Confirmed!</h4>
              <p className="text-sm text-slate-300 max-w-xs mx-auto">
                Thank you, <strong>{formData.name || 'Investor'}</strong>. We have sent the calendar invite and confidential data room access to <strong>{formData.email || 'your email'}</strong>.
              </p>
              <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 text-xs text-slate-400 text-left space-y-1">
                <div><strong>Meeting:</strong> {formData.meetingType}</div>
                <div><strong>VC / Fund:</strong> {formData.firm || 'Venture Partner'}</div>
              </div>
              <button
                onClick={resetForm}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      required
                      placeholder="e.g. Apex Climate Ventures"
                      value={formData.firm}
              ...
              <div>
                ...
                >
                  <option>30-Min Seed Pitch (Deck & Demo)</option>
                  <option>45-Min Technical Deep Dive (AI & Edge Architecture)</option>
                  <option>Campus Deployment & Pilot Partnership Walkthrough</option>
                </select>
              </div>
              ...
              <div className="pt-2">
                ...
                >
                  ...
                  <span>Confirm Pitch Briefing</span>
                </button>
              </div>
